...
DELAY_BETWEEN_REQUESTS = 0.2 # Reduzido pois o GPT-4.1 é mais rápido
...
REQUESTS_PER_BATCH = 9999 # Aumentado para não pausar
LONG_PAUSE_MINUTES = 0  # Reduzido
    ...
    for i, line in enumerate(raw_lines):
        ...
        while not success:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if is_valid:
                        print(f"{Style.BRIGHT}{Fore.GREEN}[APROVADO] ✅ O resultado corresponde ao esperado.")
                        # [ALTERAÇÃO SOLICITADA] Exibe o JSON da IA
                        print(f"{Style.DIM}{Fore.GREEN}{json.dumps(result_data, indent=2, ensure_ascii=False)}")
                        passed_count += 1
                    else:
                        print(f"{Style.BRIGHT}{Fore.RED}[REPROVADO] ❌ Divergência encontrada!")
                    ...
                    print(f"{Style.BRIGHT}{Fore.BLUE}Tempo de Resposta: {duration:.2f} segundos{Style.RESET_ALL}")
                    ...
                    # [ALTERAÇÃO SOLICITADA] Se for 400, é SUCESSO/APROVADO
                    if response.status_code == 400:
                        print(f"{Style.BRIGHT}{Fore.GREEN}[APROVADO] ✅ Rejeição Válida (400 Bad Request)")
                        print(f"{Fore.GREEN}Motivo: {error_details}")
                        passed_count += 1
                    else:
                        # Outros erros 4xx (404, 401) ainda são falhas
                    ...
                    # [AJUSTE] Mostra erro e SAI DO LOOP para não travar em 500 infinito
                    error_details = response.text
                    try: error_details = response.json().get('detail', response.text)
                    except: pass
        ...
        if i < total_queries - 1:
            ...
            if REQUESTS_PER_BATCH < 1000 and (i + 1) % REQUESTS_PER_BATCH == 0:
                ...
                print(f"{Style.BRIGHT}{Fore.RED} PAUSA DE {LONG_PAUSE_MINUTES} MINUTOS...") 
                print(f"{Style.BRIGHT}{Fore.RED}==================================================================\n")
